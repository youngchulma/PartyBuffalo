...
namespace ParrotLibs
{
    public class Entry
    {
        ...
        /// 
        /// </summary>
        /// <param name="aPartition"></param>
        /// <param name="aEntryData"></param>
        /// <param name="aDrive"></param>
        public Entry(PartitionInfo aPartition, EntryData aEntryData, Drive aDrive)
        {
            this.PartitionInfo = aPartition;
            this.EntryData = aEntryData;
            this.Drive = aDrive;
        }
        ...
        public uint[] BlocksOccupied
        {
            get
            {
                if (blocks == null)
                {
                    if (!IsDeleted)
                    {
                        EntryFunctions sEntryFuncations = new EntryFunctions(this);
                        blocks = sEntryFuncations.GetBlocksOccupied().ToList();
                    }
                    else
                    {
                        if (IsFolder)
                        ...
                        else
                        {
                            ...
                            size = VariousFunctions.UpToNearestCluster(size, this.PartitionInfo.ClusterSize);
                            blocks = new List<uint>();
        ...
        public bool IsFolder
        {
            get
            {
                if (Flags.Contains(ParrotLibs.Geometry.Flags.Directory))
                {
                    return true;
                }
                return false;
    ...
    public class File : Entry
    {
        ...
        public void Extract(string Destination)
        {
            ...
            Streams.Writer FileWriter = new ParrotLibs.Streams.Writer(new System.IO.FileStream(Destination, System.IO.FileMode.Create));
            // Loop for each block...
        ...
        public System.Drawing.Image TitleIcon()
        {
            if (IsSTFSPackage())
            {
                if (ti == null && !tiattempted)
                {
                    ...
                    Streams.Reader r = new ParrotLibs.Streams.Reader(GetStream());
        ...
        public bool IsSTFSPackage()
        {
            if (!isstfs && !stfschecked)
            {
                if (Size >= 0xA000)
                {
                    Streams.Reader r = new ParrotLibs.Streams.Reader(Drive.Stream());
                    ...
                    r = new ParrotLibs.Streams.Reader(new System.IO.MemoryStream(Buffer));
                    uint val = 0;
                    try
                    {
                        val = r.ReadUInt32();
    ...
    public class Folder : Entry
    {
                    ...
                    foreach (Folder ProfileFolder in XboxDashboard.Folders())
                    {
                        ...
                        if (ProfileFolder.IsKnownFolder && ProfileFolder.Name == "00010000")
                        {
                            ...
                            foreach (File f in ProfileFolder.Files())
                            {
                                ...
                                {
                                    Streams.Reader r = new ParrotLibs.Streams.Reader(f.GetStream());
        ...
        public Structs.WriteResult CreateNewFolder(string Name)
        {
            ...
            Folder f = new Folder(this.PartitionInfo,
                ef.GetNewEntry(this, 0, new ParrotLibs.Geometry.Flags[] { Geometry.Flags.Directory }, Name),
                this.Drive);
        ...
        public Structs.WriteResult CreateNewFile(string Path)
        {
            ...
            if (new System.IO.FileInfo(Path).Length == 0)
            {
                ...
                File f = new File(this.PartitionInfo, new EntryFunctions(this).GetNewEntry(this, 0, new ParrotLibs.Geometry.Flags[0], NewName), this.Drive);
                f.Parent = this;
                f.FullPath = this.FullPath + "\\" + f.Name;
                Return.Entry = f;
                eea.ModifiedEntry = f;
            ...
            else
            {
                ...
                EntryData newE = new EntryFunctions(this).GetNewEntry(this, (uint)new System.IO.FileInfo(Path).Length, new ParrotLibs.Geometry.Flags[0], NewName);
                ...
                try
                {
                    FileReader = new ParrotLibs.Streams.Reader(new System.IO.FileStream(Path, System.IO.FileMode.Open));
                }
                catch
                {
