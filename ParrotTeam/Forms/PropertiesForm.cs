...
namespace Party_Buffalo.Forms
{
    public partial class PropertiesForm : Form
    {
        ...
        private void LoadGeneral(Entry xFile)
        {
            ...
            if (!xFile.IsFolder)
            {
                l_size.Text = "0x" + ((File)xFile).Size.ToString("X") + " (" + ParrotLibs.VariousFunctions.ByteConversion(((File)xFile).Size) + ")";
                l_sizeOnDisk.Text = "0x" + ParrotLibs.VariousFunctions.UpToNearestCluster(((File)xFile).Size, xFile.PartitionInfo.ClusterSize).ToString("X") + " (" + ParrotLibs.VariousFunctions.ByteConversion(ParrotLibs.VariousFunctions.UpToNearestCluster(((File)xFile).Size, xFile.PartitionInfo.ClusterSize)) + ")";
            }
            else
            {
            ...
            l_Flags.Text = "";
            ParrotLibs.Geometry.Flags[] flags = xFile.Flags;
