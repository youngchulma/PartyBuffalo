...
namespace Party_Buffalo.Forms
{
    public partial class EntryAction : Form
    {
        ...
        bool Timer = true;
        ParrotLibs.Structs.Queue[] MultiInject;
        ...
        public EntryAction(ParrotLibs.Structs.Queue[] mi)
        {
            InitializeComponent();
            if (Environment.OSVersion.Version.Build >= 7600)
            {
        ...
        private void DoExisting(List<ParrotLibs.Structs.ExistingEntry> aExistList)
        {
            ...
            List<ParrotLibs.Structs.ExistingEntry> sFiles = new List<ParrotLibs.Structs.ExistingEntry>();
            List<ParrotLibs.Structs.ExistingEntry> sFolders = new List<ParrotLibs.Structs.ExistingEntry>();
            List<ParrotLibs.Structs.ExistingEntry> sFilesWithFolders = new List<ParrotLibs.Structs.ExistingEntry>();
            List<ParrotLibs.Structs.ExistingEntry> sFoldersWithFiles = new List<ParrotLibs.Structs.ExistingEntry>();
            
            MergedPaths = new List<ParrotLibs.Structs.ExistingEntry>();
            goto _Sort;
        _Sort:
            {
                // 对ExistList中的元素进行分类
                foreach (ParrotLibs.Structs.ExistingEntry sEntry in aExistList)
                {
                ...
                // 分类后，对ExistList进行初始化
                aExistList = new List<ParrotLibs.Structs.ExistingEntry>();
            }
            ...
            for (int i = 0; i < sFolders.Count; i++)
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    ...
                    if (Checked)
                    {
                        ...
                        sFolders = new List<ParrotLibs.Structs.ExistingEntry>();
                        break;
                    }
                    else
                    {
                        ...
                        {
                            ParrotLibs.Structs.WriteResult wr = ((Folder)sFolders[i].Existing).CreateNewFile(fi.FullName);
                            ParrotLibs.Structs.ExistingEntry exe = new ParrotLibs.Structs.ExistingEntry();
            ...
            for (int i = 0; i < sFiles.Count; i++)
            {
                ...
                if (Delete)
                {
                    sFiles[i].Existing.Parent.FolderAction += new ParrotLibs.Structs.FolderActionChanged(EntryAction_FolderAction);
                ...
                else
                {
                    ...
                    {
                        sFiles = new List<ParrotLibs.Structs.ExistingEntry>();
                    }
                }
            }
        ...
        public List<ParrotLibs.Structs.ExistingEntry> MergedPaths
        {
            get;
            private set;
        }

        void EntryAction_FolderAction(ref ParrotLibs.Structs.FolderAction Progress)
        {
        ...
        private void EntryAction_FileAction(ref ParrotLibs.Structs.FileAction Progress)
        {
            try
            {
                int sCurValue = Progress.Progress;
