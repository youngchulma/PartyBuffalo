            ...
            listView1.Invoke((MethodInvoker)delegate
            {
                ...
                if (stfsInfo.Checked && Parent.IsTitleIDFolder)
                {
                    if (Parent.IsKnownFolder)
                    {
                        ...
                        {
                            if (Parent.Name.ToLower() == ParrotLibs.VariousFunctions.Known[i].ToLower())
                            {
                                ListViewItem.ListViewSubItem lsi = new ListViewItem.ListViewSubItem(li, ParrotLibs.VariousFunctions.KnownEquivilent[i]);
                                li.SubItems.Add(lsi);
                                break;
                            }
                        }
